...
def export_to_onnx(model, vocab_size, device, seq_len=32, save_path='model/sudenmind.onnx'):
        ...
        with warnings.catch_warnings():
            ...
            with torch.no_grad():
                torch.onnx.export(
                    ...
                    opset_version=17,  # 使用较新的 opset，Netron 支持更好
                    dynamo=False, 
                    fallback=True,
...
class Trainer:
    def __init__(self, model, train_loader, device, vocab_size, lr=1e-3):
        ...
        self.total_steps = len(train_loader) * 500
        self.warmup_steps = int(self.total_steps * 0.2)
    ...
    def train_epoch(self, epoch_num=300, patience=20, target_loss=0.3):
        self.model.train()
        best_loss = float('inf')
        counter = 0
...
if __name__ == "__main__":
    ...
    model = SudenMind(vocab_size, 256, 512, vocab_size).to(device)
